...
const host = 'https://blog.lazyalex.com'
...
export function handleHeadMeta(context: TransformContext) {
    ...
    const ogImage: HeadConfig = ["meta", { property: "og:image", content: host + "/avatar.jpg" }]
    const twitterCard: HeadConfig = ["meta", { name: "twitter:card", content: "summary" }]
    const twitterImage: HeadConfig = ["meta", { name: "twitter:image:src", content: host + "/avatar.jpg" }]
