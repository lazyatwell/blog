...
const genArticleTag = (post: Article) => {
  const regularPath = post.regularPath
  if (regularPath.startsWith('/posts/draft/')) {
    return h('span', { class: 'tag', title: '草稿' }, '📌')
  } else if (regularPath.startsWith('/posts/private-notes/')) {
    return h('span', { class: 'tag', title: '私人笔记' }, '🔒')
  } else if (regularPath.startsWith('/posts/trash/')) {
    return h('span', { class: 'tag', title: '已删除' }, '🗑️')
  }
